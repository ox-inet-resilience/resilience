...
class TestBank:
    ...
    def test_act_lev_only(self, bank):
        ...
        # act
        bank.act()
        assert bank.get_leverage() == pytest.approx(0.05)
    ...
    def test_act_rwa_only(self, bank):
        ...
        # act
        bank.act()
        assert bank.get_leverage() == pytest.approx(0.045454545454545454547)
        # make sure that RWCR target is reached
        assert bank.get_RWA_ratio() == pytest.approx(bank.RWCR_target)
