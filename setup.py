...
extmods = [
    ...
    Extension("resilience.contracts.%s" % i, sources=["resilience/contracts/%s.py" % i])
    for i in ["TradableAsset", "AssetCollateral", "Loan", "Deposit"]
] + [
    Extension("resilience.markets.%s" % i, sources=["resilience/markets/%s.py" % i])
    for i in ["Market", "AssetMarket"]
]
